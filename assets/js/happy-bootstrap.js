(function($) {
  ...
  $.fn.isHappy = function(config) {
    ...
    function getError(error) {
      return $('<span id="' + error.id + '" class="help-inline unhappyMessage">' + error.message + '</span>');
    }
    ...
    function processField(opts, selector) {
      ...
      field.testValid = function(submit) {
        ...
        if (error) {
          controls = el.addClass('unhappy').parents('.controls');
          controls.append(errorEl);
          if (isFunction(config.afterTestValid)) config.afterTestValid();
          return false;
        } else {
          temp = errorEl.get(0);
          // this is for zepto
